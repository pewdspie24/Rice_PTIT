...
validGen = ImageDataGenerator().flow_from_directory(
    "validation",
    class_mode="sparse",
    target_size=(640, 640),
    color_mode="rgb",
...
# data_augmentation.layers[0].adapt(trainGen)
...
# image = trainGen[np.random.choice(range(trainGen.shape[0]))]
...
def run_experiment(model):
    total_steps = int((len(trainGen) / BATCH_SIZE) * EPOCHS)
    ...
    history = model.fit(
        x=trainGen,
        steps_per_epoch=len(list(paths.list_images("train"))) // BATCH_SIZE,
        batch_size=BATCH_SIZE,
        epochs=EPOCHS,
        validation_data=validGen,
        callbacks=[checkpointer],
        validation_steps=len(list(paths.list_images("validation"))) // BATCH_SIZE,
    )
